...
if [ "$REPLY" != "O" ]; then
   echo "Sortie - Abandon " exit
	...
	echo ""
        curl -N -s http://green-spider.free-solutions.org/GREEN_SPIDER.dd.zst | unzstd -T0 -c | (pv -B64M -n -  > $DEVUSB  conv=notrunc,noerror) 2>&1 | dialog --title "Creation de Free-Solutions OS sur USB" --gauge "\nCréation Clé Free-Solutions OS en cours Veuillez patienter...\n\nBien attendre jusqu'au message :\n Votre clé Bootable Free-Solutions OS est prête à être bootée  ENJOY" 10 75 0 
fi
sync
