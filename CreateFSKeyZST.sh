...
if [ "$REPLY" != "O" ]; then
   echo "Sortie - Abandon " exit
	...
	echo ""
        curl -N -s https://www.free-solutions.ch/GREEN_SPIDER/5.0/GREEN_SPIDER_5.0.4OFFICIAL.dd.zst | unzstd -c | (pv -B32M -n -  > $DEVUSB  conv=notrunc,noerror) 2>&1 | dialog --title "Creation de Free-Solutions OS sur USB" --gauge "\nCréation Clé Free-Solutions OS en cours Veuillez patienter...\n\nBien attendre jusqu'au message :\n Votre clé Bootable Free-Solutions OS est prête à être bootée  ENJOY" 10 75 0 
fi
sync
